//
//  Stokes2DGaussIntConstOpen.c
//  
//
//  Created by Giacomo on 6/23/15.
//
//
...
void Stokes2DAxisIntConst(double *x, double *y, double *x0, double *y0, double *GXX, double *GXY, double *GYX, double *GYY, double *TXXX, double *TXXY, double *TXYY, double *TYXX, double *TYXY, double *TYYY, double *PYX, double *PYY, int field, int elem)
{
  int i, j, dolog, axis;
  ...
  for (j=0; j<elem; j++) {  
      ...
      for (i=0; i<field; i++) {
          ...
          //singular treatment
          GXX[j*field+i] += 2*(-dl*log(dl/2)+dl)*(1-dolog)*(1-axis);
          GYY[j*field+i] += 2*(-dl*log(dl/2)+dl)*(1-dolog)*(1-axis);
...
Stokes2DAxisIntConst(inMatrixX, inMatrixY, inMatrixX0, inMatrixY0, GXX, GXY, GYX, GYY, TXXX, TXXY, TXYY, TYXX, TYXY, TYYY, PYX, PYY, ncols, nrows-1);
}