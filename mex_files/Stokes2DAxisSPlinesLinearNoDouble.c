...
void Stokes2DAxisSPlinesLinearNoDouble(double *aax, double *bbx, double *ccx, double *ddx, double *aay, double *bby, double *ccy, double *ddy, double *x0, double *y0, double *GXX, double *GXY, double *GYX, double *GYY, double *A11, double *A12, double *A21, double *A22, int field, int elem)
{
  int i, j, dolog1, dolog2, axis;
  double SXX, SXY, SYX, SYY;    //single layer
  double QXXX, QXXY, QXYY, QYXX, QYXY, QYYY, tempQ11, tempQ12, tempQ21, tempQ22;    //double layer
  ...
  for (j=0; j<elem; j++) {  
      ...
      for (i=0; i<field; i++) {
          ...
          for (int l=0; l<6; l++) {
                ...
                //double layer with normal vector
                tempQ11 = QXXX*nx+QXXY*ny;
                tempQ12 = QXXY*nx+QXYY*ny;
                tempQ21 = QYXX*nx+QYXY*ny;
                tempQ22 = QYXY*nx+QYYY*ny;
                ...
                A11a[j*field+i] += phiA*GW[l]*tempQ11/2 * h;
                A12a[j*field+i] += phiA*GW[l]*tempQ12/2 * h;
                A21a[j*field+i] += phiA*GW[l]*tempQ21/2 * h;
                A22a[j*field+i] += phiA*GW[l]*tempQ22/2 * h;
                ...
                A11b[j*field+i] += phiB*GW[l]*tempQ11/2 * h;
                A12b[j*field+i] += phiB*GW[l]*tempQ12/2 * h;
                A21b[j*field+i] += phiB*GW[l]*tempQ21/2 * h;
                A22b[j*field+i] += phiB*GW[l]*tempQ22/2 * h;
...
Stokes2DAxisSPlinesLinearNoDouble(ax, bx, cx, dx, ay, by, cy, dy, inMatrixX0, inMatrixY0, GXX, GXY, GYX, GYY, A11, A12, A21, A22, ncols, nrows-1);
}
