...
...
  = kernelsAxisDipolesAndDoublet(X,Y,X0,Y0)
...
%------
  ...
  QXXXD = - Y6DX .* (-3.0*RJ50 + 5.0*DX2.*RJ70);
  QXXYD = - Y66  .* (-Y.*RJ50+Y0.*RJ51 - 5.0*DX2.*(-Y.*RJ70+Y0.*RJ71));
  QXYXD =   QXXYD;
  QXYYD = - Y6DX .* (-RJ50 + 5.0*(Y2.*RJ70-2.0*YYP.*RJ71+Y02.*RJ72));
  QYXXD =   Y66  .* (-Y.*RJ51+Y0.*RJ50 - 5.0*DX2.*(-Y.*RJ71+Y0.*RJ70));
  QYXYD =   Y6DX .* (-RJ51 + 5.0*YY2.*RJ71 ...
                              - 5.0*YYP.*(RJ70+RJ72));
  QYYXD = QYXYD;
  QYYYD =    Y66.*(-3.0D0*Y.*RJ51+Y0.*(RJ50+2.0D0*RJ52) ...
                      +5.0D0*Y.*Y02.*RJ73 ...
  ...
  QXXXSD = DX.*QXXXD + FQXXX;
  QXXYSD = DX.*QXXYD + FQXXY;
  QXYXSD = DX.*QXYXD + FQXYX;
  QYXXSD = DX.*QYXXD + FQYXX;
  QXYYSD = DX.*QXYYD + FQXYY;
  QYXYSD = DX.*QYXYD + FQYXY;
  QYYXSD = DX.*QYYXD + FQYYX;
  QYYYSD = DX.*QYYYD + FQYYY;
