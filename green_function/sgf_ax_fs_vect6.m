...
...
   ...
   ,Iaxis] ...
...
  = sgf_ax_fs_vect6 (Iopt,X,Y,X0,Y0)
...
%                  -8.0*RL52 + RL50);
 ...
 %tryI52 = -RK.^5./8./(Y.*Y0).^(5/2).*(-(8-8*RK2-RK4)./3./(1-RK2).*F + (2*RK6-12.*RK2+8)./3./(1-RK2).^2.*E)./RK4;
 %myRL56 = -1/3./RK6.*((3*RK4+8-13*RK2)./(1-RK2).^2.*E + (9*RK2-8)./(1-RK2).*F);
 %myRJ53 = -1./24./(Y.*Y0).^(5/2)./RK.*(-(1+RK).*(RK2-2).*(RK4+32*RK2-32)./(1-RK2).*F - (2*RK4.^2+2*RK6-66*RK4+128.*RK2-64)./(1-RK2).^2.*E);
 %myRJ53 = RK.^5./8./(Y.*Y0).^(5/2).*((RK6+30*RK4-96*RK2+64)./(3*RK6.*(1-RK2)).*F + (2*RK4.^2+2*RK6-66*RK4+128*RK2-64)./(3*RK6.*(1-RK2).^2).*E);
