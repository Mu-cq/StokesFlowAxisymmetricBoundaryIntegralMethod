...
thetaCone = 2*pi/180;          % inclinitation of the cone
Hcc = 0.1;
beta = 1;
bubbleNucleates = 20;
bubbleCycles = 3;
...
endCycle = 3;   %if 1 stops when center of mass exit the cone, if 2 stops when second bubble nucleates, 3 take out the one which is inflating less when a thiid one is nucleating
...
%saving options
SaveHowMany = 10000;                                  % output how many times
...
for i = 1:bubbleCycles
    ...
    if nBubble==1
        ...
        outFun = @(t,var,flag) eventBlocksODE(t,var,flag,tParametricBase,f1bubble,beta,Hcc,endCycle,PARAM);
    ...
    elseif nBubble==2
        ...
        %output and event function
        outFun = @(t,var,flag) eventBlocksTwoBubblesODE(t,var,flag,tParametricBase,f2bubbles,endCycle,PARAM);
    ...
    [breakNow,initial,tParametricBase,Tsave,PARAM] = initialConditionNextCycle(tParametricBase,nBubble,manyT{i},manyY{i},fToSolve,Tsave,Tend,SaveHowMany,beta,Hcc,endCycle,PARAM);
