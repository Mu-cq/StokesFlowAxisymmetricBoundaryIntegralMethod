...
if oneline==0
    ...
    %yyy = linspace(0,0.05,meshSize);
    xxx = linspace(0,6,meshSize);
...
PARAM.typeBCstokes = 5;           % 1 is prescribed normal velocity, 2 is prescibed normal stress, 3 is prescribed tangent velocity
...
if PARAM.typeBCstokes(1)==0
    Flubrication = 6*pi*rDot*log(manyGap);
    typeBC = 'rigid shell';
    myYlabel = 'F^{\lambda=0}/(a\dot{a})';
elseif PARAM.typeBCstokes(1)==5
    Flubrication = -1.5*pi*rDot*log(manyGap);
    typeBC = 'bubble';
    myYlabel = 'F^{\lambda \rightarrow \infty}/(a\dot{a})';
end

%plot gap
figure
if numel(manyGap)>20
    semilogx(manyGap,F1,'-k')
else
    semilogx(manyGap,F1,'.k','Markersize',30)
end
hold on
semilogx(manyGap,Flubrication,'--')
...
if oneline==1||oneline==0
    ...
    for kkk = 1:row
        [X0(kkk,:),Y0(kkk,:),uxGrid(kkk,:),uyGrid(kkk,:),pressure(kkk,:)] = computeVelPressField(X0(kkk,:),Y0(kkk,:),x,y,yStokes,0,0,PARAM,subPoint,coeffSub,1);
    end
...
if oneline==0
    ...
    figure
    plotFieldAxis(x,y,X0,Y0,uxGrid,uyGrid,1.5,0.1,1,90,PARAM)
    %axis equal
...
if numel(manyGap)==1
    ...
    disp(['Force acting on the sphere is F=' num2str(F1)])
