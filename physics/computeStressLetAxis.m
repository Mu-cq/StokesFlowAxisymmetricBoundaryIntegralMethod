...
function S = computeStressLetAxis(x,y,vSlip,range,PARAM)
...
for i = range
    ...
    if PARAM.orderGeometry(i)==0 && PARAM.orderVariable(i)==0
        ...
        %vSlipHereX = vSlip(startMatrix:endMatrix).*nx';
        %vSlipHereY = vSlip(startMatrix:endMatrix).*ny';
               
        %force on panel
        toInt = 2*vSlip(startMatrix:endMatrix);
