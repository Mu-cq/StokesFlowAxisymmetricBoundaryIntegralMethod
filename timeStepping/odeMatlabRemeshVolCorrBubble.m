...
function [T,Y] = odeMatlabRemeshVolCorrBubble(f,TTT,initial,event,remeshInLoop,volCorrInLoop,options,ODE,PARAM)
...
while Tvect(end)<TTT(end)
    ...
    elseif ODE==3
        [Tvect,Yvect] = ode23s(f,TTT,x0,options);
    elseif ODE==4
        [Tvect,Yvect] = ode15s(f,TTT,x0,options);
    else
