...
function [a,b]=boundaries_const(R1,R2,L,n,m,j,q)

     no = R1/R2;
     %no = 0.5;

     %build the panel at the outlet
     [a(1:ceil(n*no)+1),b(1:ceil(n*no)+1)]=drawline2(L,0,L,R1,ceil(n*no));
     
     %build the panel at the outlet
     [a(ceil(n*no)+1:n+1),b(ceil(n*no)+1:n+1)]=drawline2(L,R1,L,R2,n-ceil(n*no));

     %build the panel at the wall
     [a(n+1:m+n+1),b(n+1:m+n+1)]=drawline2(L,R2,0,R2,m);

     %build the panel at the inlet
     [a(n+m+1:n+m+ceil(j*(1-no))+1),b(n+m+1:n+m+ceil(j*(1-no))+1)]=drawline2(0,R2,0,R1,ceil(j*(1-no)));
     
     %build the panel at the inlet
     [a(n+m+ceil(j*(1-no))+1:n+m+j+1),b(n+m+ceil(j*(1-no))+1:n+m+j+1)]=drawline2(0,R1,0,0,j-ceil(j*(1-no)));
