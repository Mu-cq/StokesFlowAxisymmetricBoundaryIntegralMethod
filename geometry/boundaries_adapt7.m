...
function [a,b]=boundaries_adapt7(PARAM)

    m1= round(0.9*PARAM.m);     m2 = PARAM.m-m1;    m = PARAM.m;    q = PARAM.q;
    L = PARAM.L;
    theta = PARAM.theta;
    R = PARAM.R;
    g1 = PARAM.tune_wall;
...
%      end         
     ...
     x1 = a(m1+1);
     y1 = b(m1+1) + R/10;
     a(m1+1:m1+m2+1) = R/10.*cos(-phi*pi/2+3*pi/2)+x1;
     b(m1+1:m1+m2+1) = R/10.*sin(-phi*pi/2+3*pi/2)+y1;
