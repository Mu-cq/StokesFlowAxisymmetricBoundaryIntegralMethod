...
function [a_out,b_out,V_after,Area_after] = raise_volume(PARAM,a_in,b_in,V_in)

        %compute area and volume
        Area_before = surf_gauss_vect(a_in,b_in);
        V_before = axis_int_gauss_vect(a_in,b_in)/V_in;

        %%%%%%%%%%%%% VOLUME CORRECTION %%%%%%%%%%%%%%%%%%%
        
        for i = 1:PARAM.how_many
            ...
            a_in = a_in-3*(V_before-1-PARAM.raise)/Area_before*N(1,:);
            b_in = b_in-3*(V_before-1-PARAM.raise)/Area_before*N(2,:);
