...
function [x,y] = draw_test(x1,y1,R,N)
    
    %create the empty matrix
    theta1 = -pi/2:pi/(N/4):pi/2;
    theta2 = pi/2:pi/(N/4):3*pi/2;

    %build the semicircles (half)
    X1 = R.*cos(theta1)+x1+R;
    Y1 = R.*sin(theta1)+y1;
    X3 = R.*cos(theta2)+x1-R;
    Y3 = R.*sin(theta2)+y1;
    
    %build straigths parts
    [X2,Y2]=drawline2(X1(end),Y1(end),X3(1),Y3(1),N/4);
