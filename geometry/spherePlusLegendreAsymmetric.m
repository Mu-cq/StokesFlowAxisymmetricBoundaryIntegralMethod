...
function [x,y] = spherePlusLegendreAsymmetric(t,delta,PARAM)

%legendre
P3 = legendre(3,cos(pi*t));
P3 = P3(1,:)';

%draw
x = (1+delta*P3).*cos(pi*t);
y = (1+delta*P3).*sin(pi*t);
...
while abs(V0-V)>1e-15
   ...
   epsilon = (eps1+eps2)/2;
   x = epsilon*(1+delta*P3).*cos(pi*t);
   y = epsilon*(1+delta*P3).*sin(pi*t);
   V = VolumeCurvilinearAxisSpectral(x,y,PARAM);
