...
function [GXX,GXY,GYX,GYY,A11,A12,A21,A22,T1,T2,D1,D2,Iaxis] = computeGT_spline3(x,y,ax,ay,bx,by,cx,cy,dx,dy)
...
%     SYY(2:end,6:6:end) = SYY(2:end,6:6:end) + diag(2*log(sqrt((globalX(6:6:end)-XX(12:6:end)).^2+(globalY(6:6:end)-YY(12:6:end)).^2))-1);
    ...
    for i = 1:N
        for k = 1:el
            GXX(i,k) = GXX(i,k) + SXX(i,1+6*(k-1):6*k).*phia*GW';
            GXX(i,k+1) = SXX(i,1+6*(k-1):6*k).*phib*GW';
            
            GXY(i,k) = GXY(i,k) + SXY(i,1+6*(k-1):6*k).*phia*GW';
            GXY(i,k+1) = SXY(i,1+6*(k-1):6*k).*phib*GW';
            
            GYX(i,k) = GYX(i,k) + SYX(i,1+6*(k-1):6*k).*phia*GW';
            GYX(i,k+1) = SYX(i,1+6*(k-1):6*k).*phib*GW';
            
            GYY(i,k) = GYY(i,k) + SYY(i,1+6*(k-1):6*k).*phia*GW';
            GYY(i,k+1) = SYY(i,1+6*(k-1):6*k).*phib*GW';
            
            A11(i,k) = A11(i,k) + (QXXX(i,1+6*(k-1):6*k).*nx(6*(k-1)+1:k*6) + QXXY(i,1+6*(k-1):6*k).*ny(6*(k-1)+1:k*6)).*phia*GW';
            A11(i,k+1) = (QXXX(i,1+6*(k-1):6*k).*nx(6*(k-1)+1:k*6) + QXXY(i,1+6*(k-1):6*k).*ny(6*(k-1)+1:k*6)).*phib*GW';
            
            A12(i,k) = A12(i,k) + (QXYX(i,1+6*(k-1):6*k).*nx(6*(k-1)+1:k*6) + QXYY(i,1+6*(k-1):6*k).*ny(6*(k-1)+1:k*6)).*phia*GW';
            A12(i,k+1) = (QXYX(i,1+6*(k-1):6*k).*nx(6*(k-1)+1:k*6) + QXYY(i,1+6*(k-1):6*k).*ny(6*(k-1)+1:k*6)).*phib*GW';
            
            A21(i,k) = A21(i,k) + (QYXX(i,1+6*(k-1):6*k).*nx(6*(k-1)+1:k*6) + QYXY(i,1+6*(k-1):6*k).*ny(6*(k-1)+1:k*6)).*phia*GW';
            A21(i,k+1) = (QYXX(i,1+6*(k-1):6*k).*nx(6*(k-1)+1:k*6) + QYXY(i,1+6*(k-1):6*k).*ny(6*(k-1)+1:k*6)).*phib*GW';
            
            A22(i,k) = A22(i,k) + (QYYX(i,1+6*(k-1):6*k).*nx(6*(k-1)+1:k*6) + QYYY(i,1+6*(k-1):6*k).*ny(6*(k-1)+1:k*6)).*phia*GW';
            A22(i,k+1) = (QYYX(i,1+6*(k-1):6*k).*nx(6*(k-1)+1:k*6) + QYYY(i,1+6*(k-1):6*k).*ny(6*(k-1)+1:k*6)).*phib*GW';
            
            T1(i,k) = (QXXX(i,1+6*(k-1):6*k).*nx(6*(k-1)+1:k*6) + QXXY(i,1+6*(k-1):6*k).*ny(6*(k-1)+1:k*6))*GW';
            T2(i,k) = (QYXX(i,1+6*(k-1):6*k).*nx(6*(k-1)+1:k*6) + QYXY(i,1+6*(k-1):6*k).*ny(6*(k-1)+1:k*6))*GW';
            
            D1(i,k) = (PXX(i,1+6*(k-1):6*k).*nx(6*(k-1)+1:k*6) + PXY(i,1+6*(k-1):6*k).*ny(6*(k-1)+1:k*6))*GW';
            D2(i,k) = (PYX(i,1+6*(k-1):6*k).*nx(6*(k-1)+1:k*6) + PYY(i,1+6*(k-1):6*k).*ny(6*(k-1)+1:k*6))*GW';
        end
    end
    ...
    for i = 2:N-2
       ...
       GXX(i,i) = GXX(i,i) + 3/2*h0(i);
       GXX(i,i+1) = GXX(i,i+1) + 1/2*h0(i);
       GXX(i+1,i) = GXX(i+1,i) + 1/2*h1(i);
       GXX(i+1,i+1) = GXX(i+1,i+1) + 3/2*h1(i);
       
       GYY(i,i) = GYY(i,i) + 3/2*h0(i);
       GYY(i,i+1) = GYY(i,i+1) + 1/2*h0(i);
       GYY(i+1,i) = GYY(i+1,i) + 1/2*h1(i);
       GYY(i+1,i+1) = GYY(i+1,i+1) + 3/2*h1(i);
