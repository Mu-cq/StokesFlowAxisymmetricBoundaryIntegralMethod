...
function z = nearly_sing_1layer2Ddrop_DROP_WALL(X_wall,Y_wall,K,N,PARAM,ax,ay,bx,by,cx,cy,dx,dy)
    ...
    %here is better to treat 1/m x(y)
    %m = ny./nx;
    mINV = nx./ny;
    ...
    for i = 1:numel(X_wall)
        ...
        RES = mINV.*(Y_wall(i)-y)-(X_wall(i)-x);
        [~,ind] = min(abs(RES));
        
        %projection point
        Ks = ((bxxx(ind)+2*cxxx(ind)*ttt(ind)+3*dxxx(ind)*ttt(ind)^2)*(2*cyyy(ind)+6*dyyy(ind)*ttt(ind))-...
...
%         plot(x,y)
%         hold on
%         plot([X_wall(i) xNOW],[Y_wall(i) yNOW],'k')
%         axis equal
%         xlabel('x')
%         ylabel('r')
%         grid on
