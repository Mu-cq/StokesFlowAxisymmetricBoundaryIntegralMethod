...
function [A,b,nnx,nny] = buildInfluenceMatrixAndKnownTerm2d(GXX,GXY,GYX,GYY,A11,A12,A21,A22,Xsing,nnn,x,y,PARAM)
  ...
  for i = 1:numel(PARAM.n)
      ...
      if PARAM.typeBC(i)==0||PARAM.typeBC(i)==1||PARAM.typeBC(i)==3||PARAM.typeBC(i)==4||PARAM.typeBC(i)==6    % prescribe velocity
          ...
          if PARAM.panelType(i)==0 || PARAM.panelType(i)==1 % wall
                ...
                U(2*startMatrix-1:2*endMatrix,2*startMatrix-1:2*endMatrix) = U(2*startMatrix-1:2*endMatrix,2*startMatrix-1:2*endMatrix) - diag(2*pi*ones(2*nnn(i),1));
          ...
          if PARAM.typeBC(i)==0        % axial and radial velocity
                ...
                v(2*startMatrix-1:2:2*endMatrix-1) = PARAM.velBCx{i};
                v(2*startMatrix:2:2*endMatrix) = PARAM.velBCy{i};
      ...
      elseif PARAM.typeBC(i)==2    %prescribe normal stresses due to surface tension
          ...
          %add term on the diagonal
          A(2*startMatrix-1:2*endMatrix,2*startMatrix-1:2*endMatrix) = A(2*startMatrix-1:2*endMatrix,2*startMatrix-1:2*endMatrix) - diag(2*pi*ones(2*nnn(i),1))*(1+visc);
      ...
      elseif PARAM.typeBC(i)==5 || PARAM.typeBC(i)==9   %prescribe normal velocity and tangent stress or viceversa
          ...
          if PARAM.typeBC(i)==5
              ...
              A(2*startMatrix-1:2:2*endMatrix-1,2*startMatrix:2:2*endMatrix) = A(2*startMatrix-1:2:2*endMatrix-1,2*startMatrix:2:2*endMatrix) + diag(2*pi*ones(nnn(i),1)).*sinT(startMatrix:endMatrix,:);
              A(2*startMatrix:2:2*endMatrix,2*startMatrix:2:2*endMatrix) = A(2*startMatrix:2:2*endMatrix,2*startMatrix:2:2*endMatrix) - diag(2*pi*ones(nnn(i),1)).*cosT(startMatrix:endMatrix,:);
              U(2*startMatrix-1:2:2*endMatrix-1,2*startMatrix-1:2:2*endMatrix-1) = U(2*startMatrix-1:2:2*endMatrix-1,2*startMatrix-1:2:2*endMatrix-1) - diag(2*pi*ones(nnn(i),1)).*cosT(startMatrix:endMatrix,:);
              U(2*startMatrix:2:2*endMatrix,2*startMatrix-1:2:2*endMatrix-1) = U(2*startMatrix:2:2*endMatrix,2*startMatrix-1:2:2*endMatrix-1) - diag(2*pi*ones(nnn(i),1)).*sinT(startMatrix:endMatrix,:);

              %known BC
              v(2*startMatrix-1:2:2*endMatrix-1) = PARAM.velBC{i};
              v(2*startMatrix:2:2*endMatrix) = PARAM.stressBC{i};
          
          elseif PARAM.typeBC(i)==9
              
              %double layer and single layer potential
              U(1:2:end-1-addSize,2*startMatrix-1:2:2*endMatrix-1) = -SL11;
              U(1:2:end-1-addSize,2*startMatrix:2:2*endMatrix) = DL12;
              U(2:2:end-addSize,2*startMatrix-1:2:2*endMatrix-1) = -SL21;
              U(2:2:end-addSize,2*startMatrix:2:2*endMatrix) = DL22;
              A(1:2:end-1-addSize,2*startMatrix-1:2:2*endMatrix-1) = DL11;
              A(1:2:end-1-addSize,2*startMatrix:2:2*endMatrix) = -SL12;
              A(2:2:end-addSize,2*startMatrix-1:2:2*endMatrix-1) = DL21;
              A(2:2:end-addSize,2*startMatrix:2:2*endMatrix) = -SL22;

              %add singular contribution
              U(2*startMatrix-1:2:2*endMatrix-1,2*startMatrix:2:2*endMatrix) = U(2*startMatrix-1:2:2*endMatrix-1,2*startMatrix:2:2*endMatrix) + diag(2*pi*ones(nnn(i),1)).*sinT(startMatrix:endMatrix,:);
              U(2*startMatrix:2:2*endMatrix,2*startMatrix:2:2*endMatrix) = U(2*startMatrix:2:2*endMatrix,2*startMatrix:2:2*endMatrix) - diag(2*pi*ones(nnn(i),1)).*cosT(startMatrix:endMatrix,:);
              A(2*startMatrix-1:2:2*endMatrix-1,2*startMatrix-1:2:2*endMatrix-1) = A(2*startMatrix-1:2:2*endMatrix-1,2*startMatrix-1:2:2*endMatrix-1) - diag(2*pi*ones(nnn(i),1)).*cosT(startMatrix:endMatrix,:);
              A(2*startMatrix:2:2*endMatrix,2*startMatrix-1:2:2*endMatrix-1) = A(2*startMatrix:2:2*endMatrix,2*startMatrix-1:2:2*endMatrix-1) - diag(2*pi*ones(nnn(i),1)).*sinT(startMatrix:endMatrix,:);

              %known BC
              v(2*startMatrix-1:2:2*endMatrix-1) = PARAM.stressBC{i};
              v(2*startMatrix:2:2*endMatrix) = PARAM.velBC{i};
      ...
      elseif PARAM.typeBC(i)==7    % prescribe stress in x and velocity in y
          ...
          if PARAM.panelType(i)==0 || PARAM.panelType(i)==1 % wall
                ...
                A(2*startMatrix-1:2:2*endMatrix,2*startMatrix-1:2:2*endMatrix) = A(2*startMatrix-1:2:2*endMatrix,2*startMatrix-1:2:2*endMatrix) - diag(2*pi*ones(nnn(i),1));
                U(2*startMatrix:2:2*endMatrix,2*startMatrix:2:2*endMatrix) = U(2*startMatrix:2:2*endMatrix,2*startMatrix:2:2*endMatrix) - diag(2*pi*ones(nnn(i),1));
                
          end
          
          %impose axial velocity
          v(2*startMatrix-1:2:2*endMatrix-1) = PARAM.stressBCx{i};
          v(2*startMatrix:2:2*endMatrix) = PARAM.velBCy{i};
          
      elseif PARAM.typeBC(i)==8    %prescribe normal and tangent stresses
          
          %compute normal and tangent vector
          [nx,ny] = computeNormalVector(x{i},y{i},PARAM.orderVariable(i),PARAM.orderGeometry(i),PARAM.SPlinesType(i));
          tx = -ny;   ty = nx;
          
          %double layer and single layer potential
          A(1:2:end-1-addSize,2*startMatrix-1:2:2*endMatrix-1) = A11{i};
          A(1:2:end-1-addSize,2*startMatrix:2:2*endMatrix) = A12{i};
          A(2:2:end-addSize,2*startMatrix-1:2:2*endMatrix-1) = A21{i};
          A(2:2:end-addSize,2*startMatrix:2:2*endMatrix) = A22{i};
          U(1:2:end-1-addSize,2*startMatrix-1:2:2*endMatrix-1) = -GXX{i};
          U(1:2:end-1-addSize,2*startMatrix:2:2*endMatrix) = -GXY{i};
          U(2:2:end-addSize,2*startMatrix-1:2:2*endMatrix-1) = -GYX{i};
          U(2:2:end-addSize,2*startMatrix:2:2*endMatrix) = -GYY{i};
          ...
          %add term on the diagonal
          A(2*startMatrix-1:2*endMatrix,2*startMatrix-1:2*endMatrix) = A(2*startMatrix-1:2*endMatrix,2*startMatrix-1:2*endMatrix) - diag(2*pi*ones(2*nnn(i),1));
          
          %known BC
          v(2*startMatrix-1:2:2*endMatrix-1) = PARAM.stressBCnormal{i}.*nx + PARAM.stressBCtangent{i}.*tx;
          v(2*startMatrix:2:2*endMatrix) = PARAM.stressBCnormal{i}.*ny + PARAM.stressBCtangent{i}.*ty;
  ...
  if PARAM.addFlow==1
      error('Not implemented')
    under(1:2:end-1-addSize) = 4*pi*PARAM.Uunder;
