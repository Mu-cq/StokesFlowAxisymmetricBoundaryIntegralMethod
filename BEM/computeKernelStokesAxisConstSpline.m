...
function [GXX,GXY,GYX,GYY,A11,A12,A21,A22] = computeKernelStokesAxisConstSpline(x0,y0,indIn,indOut,ST,ax,ay,bx,by,cx,cy,dx,dy)
...
%     grid on
    ...
    xp0 = bx+2*cx*0.5+3*dx*0.5^2;
    yp0 = by+2*cy*0.5+3*dy*0.5^2;
    h0 = sqrt(xp0.^2+yp0.^2);
    manyh = repmat(h,N,1)';
...
%     plot(h,'o-')
    ...
    if PARAM.kernelFreeSpace==1     %free space
    ...
    if ST==1
        ...
        range = indIn:indOut;
        [SXX(:,range),SYY(:,range),anX,anY] = stokesSTconstSpline(SXX(:,range),SYY(:,range),eta,beta,h,h0,x0(range),y0(range),GPt);
    ...
    if ST==1
        ...
        GXX(range,range) = GXX(range,range) + anX;
        GYY(range,range) = GYY(range,range) + anY;
