...
function [GXX,GXY,GYX,GYY,A11,A12,A21,A22] = computeKernelStokesAxisConst(x,y,x0,y0,indIn,indOut,ST,kernelFreeSpace,wallPos)
    ...
    %mutuply kernel of doublelayer times the normal
    ...
    if ST==1
        ...
        range = indIn:indOut;
        if kernelFreeSpace==1 || kernelFreeSpace==2    %free spacea and wall bnounded has the same type of singualrities
            [SXX(:,range),SYY(:,range),SXX_an,SYY_an] = stokesST(SXX(:,range),SYY(:,range),deltaL,globalXmatr(:,range),globalYmatr(:,range),x0(range),y0(range));
        else
            error('Not implemented')
        end
    ...
    if ST==1
        GXX(range,:) = GXX(range,:) + SXX_an;
        GYY(range,:) = GYY(range,:) + SYY_an;
    end
