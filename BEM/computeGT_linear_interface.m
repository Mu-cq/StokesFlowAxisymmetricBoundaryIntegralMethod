...
function [GXX,GXY,GYX,GYY,A11,A12,A21,A22,T1,T2,D1,D2,Iaxis] = computeGT_linear_interface(x,y,walls)
...
%     SYY(2:end,6:6:end) = SYY(2:end,6:6:end) + diag(2*log(sqrt((globalX(6:6:end)-XX(12:6:end)).^2+(globalY(6:6:end)-YY(12:6:end)).^2))-1);
    ...
    for i = 1:N
        for k = 1:N-1
            GXX(i,k) = GXX(i,k) + SXX(i,1+6*(k-1):6*k).*phia*GW'*deltaL(k)/2;
            GXX(i,k+1) = SXX(i,1+6*(k-1):6*k).*phib*GW'*deltaL(k)/2;
            
            GXY(i,k) = GXY(i,k) + SXY(i,1+6*(k-1):6*k).*phia*GW'*deltaL(k)/2;
            GXY(i,k+1) = SXY(i,1+6*(k-1):6*k).*phib*GW'*deltaL(k)/2;
            
            GYX(i,k) = GYX(i,k) + SYX(i,1+6*(k-1):6*k).*phia*GW'*deltaL(k)/2;
            GYX(i,k+1) = SYX(i,1+6*(k-1):6*k).*phib*GW'*deltaL(k)/2;
            
            GYY(i,k) = GYY(i,k) + SYY(i,1+6*(k-1):6*k).*phia*GW'*deltaL(k)/2;
            GYY(i,k+1) = SYY(i,1+6*(k-1):6*k).*phib*GW'*deltaL(k)/2;
            
            A11(i,k) = A11(i,k) + (QXXX(i,1+6*(k-1):6*k)*r(1,k) + QXXY(i,1+6*(k-1):6*k)*r(2,k)).*phia*GW'*deltaL(k)/2;
            A11(i,k+1) = (QXXX(i,1+6*(k-1):6*k)*r(1,k) + QXXY(i,1+6*(k-1):6*k)*r(2,k)).*phib*GW'*deltaL(k)/2;
            
            A12(i,k) = A12(i,k) + (QXYX(i,1+6*(k-1):6*k)*r(1,k) + QXYY(i,1+6*(k-1):6*k)*r(2,k)).*phia*GW'*deltaL(k)/2;
            A12(i,k+1) = (QXYX(i,1+6*(k-1):6*k)*r(1,k) + QXYY(i,1+6*(k-1):6*k)*r(2,k)).*phib*GW'*deltaL(k)/2;
            
            A21(i,k) = A21(i,k) + (QYXX(i,1+6*(k-1):6*k)*r(1,k) + QYXY(i,1+6*(k-1):6*k)*r(2,k)).*phia*GW'*deltaL(k)/2;
            A21(i,k+1) = (QYXX(i,1+6*(k-1):6*k)*r(1,k) + QYXY(i,1+6*(k-1):6*k)*r(2,k)).*phib*GW'*deltaL(k)/2;
            
            A22(i,k) = A22(i,k) + (QYYX(i,1+6*(k-1):6*k)*r(1,k) + QYYY(i,1+6*(k-1):6*k)*r(2,k)).*phia*GW'*deltaL(k)/2;
            A22(i,k+1) = (QYYX(i,1+6*(k-1):6*k)*r(1,k) + QYYY(i,1+6*(k-1):6*k)*r(2,k)).*phib*GW'*deltaL(k)/2;
