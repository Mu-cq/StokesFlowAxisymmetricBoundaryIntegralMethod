...
function [SL,DL,SLpressure,DLpressure,fBC,uBC] = assemblySingkeAndDoubleLayer2d(solution,GXX,GXY,GYX,GYY,A11,A12,A21,A22,PX,PY,PI1,PI2,Xsing,nnn,x,y,PARAM)
...
for i = 1:numel(PARAM.n)
      ...
      elseif PARAM.typeBCstokes(i)==5 || PARAM.typeBC(i)==9
      ...
      elseif PARAM.typeBC(i)==2    % prescribe stresses, velocity are from the solution
      ...
      elseif PARAM.typeBC(i)==7    % prescribe horizontal stress and vertical velocity
          
          %stress BC
          fBC(2*startMatrix-1:2:2*endMatrix-1) = PARAM.stressBCx{i};
          fBC(2*startMatrix:2:2*endMatrix) = solution(2*startMatrix:2:2*endMatrix);
          
          %velocity BC
          uBC(2*startMatrix-1:2:2*endMatrix-1) = solution(2*startMatrix-1:2:2*endMatrix-1);
          uBC(2*startMatrix:2:2*endMatrix) = PARAM.velBCy{i};
      ...
      elseif PARAM.typeBC(i)==8    % prescribe normal and tangent stress
          ...
          %stress BC
          fBC(2*startMatrix-1:2:2*endMatrix-1) = PARAM.stressBCnormal{i}.*nx + PARAM.stressBCtangent{i}.*tx;
          fBC(2*startMatrix:2:2*endMatrix) = PARAM.stressBCnormal{i}.*ny + PARAM.stressBCtangent{i}.*ty;
          
          %velocity BC
          uBC(2*startMatrix-1:2:2*endMatrix-1) = solution(2*startMatrix-1:2:2*endMatrix-1);
          uBC(2*startMatrix:2:2*endMatrix) = solution(2*startMatrix:2:2*endMatrix);
          
      elseif PARAM.typeBC(i)==9    % prescribe normal stress and tangent velocity
          ...
          %BC from the result
          uBC(2*startMatrix-1:2:2*endMatrix-1) = solution(2*startMatrix-1:2:2*endMatrix-1);
          uBC(2*startMatrix:2:2*endMatrix) = PARAM.velBC{i};
