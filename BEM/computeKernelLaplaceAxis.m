...
function [G1,G2,Gx,Gy] = computeKernelLaplaceAxis(x,y,x0,y0,indIn,indOut,ST)
...
%     hold off
    ...
    if ST==1
        range = indIn:indOut;
        [PHI(:,range),PHIX(:,range),PHIY(:,range),P_an] = laplaceST(PHI(:,range),PHIX(:,range),PHIY(:,range),deltaL,globalXmatr(:,range),globalYmatr(:,range),x0(range),y0(range));
        %P_an = P_an.*
    end
