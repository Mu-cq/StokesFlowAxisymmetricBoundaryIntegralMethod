...
function [GXX,GXY,GYX,GYY] = computeH_spline_vect(x,y,ax,ay,bx,by,cx,cy,dx,dy,D1)
    ...
    ddeta = 2*ccx+6*ddx.*GPglobal;
    ddbeta = 2*ccy+6*ddy.*GPglobal;
    ...
    %compute curvature
    k1 = -(ddeta.*dbeta-ddbeta.*deta)./(deta.^2+dbeta.^2).^1.5;
    k2 = ny./beta;
    k = k1+k2;
...
%     plot(h,'o-')
    ...
    %compute arc lenght
    l = arclenghtGaussVect(eta,beta)';
    dkds = D1*k';
    dlds = D1*l;
    dkdl = dkds./dlds;
    dkdl = repmat(dkdl,1,N);
    ...
    %modify G
    SXX = manyh.*SXX.*dkdl.*R1;
    SXY = manyh.*SXY.*dkdl.*R2;
    SYX = manyh.*SYX.*dkdl.*R1;
    SYY = manyh.*SYY.*dkdl.*R2;
