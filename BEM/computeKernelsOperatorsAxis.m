...
function [GXX,GXY,GYX,GYY,A11,A12,A21,A22] = computeKernelsOperatorsAxis(Xsing,Ysing,nnn,x,y,PARAM)
  ...
  elseif PARAM.cfunction == 1
      ...
      for i = 1:numel(PARAM.n)
          ...
          if PARAM.orderVariable(i)==1 && PARAM.orderGeometry(i)==1     % integration on linear, curved element

              if PARAM.kernelFreeSpace==1 && PARAM.STstokes==1
                    ...
                    %compute Kernels
                    [GXX{i},GXY{i},GYX{i},GYY{i},A11{i},A12{i},A21{i},A22{i}] = computeKernelStokesAxisLinearSplineWithC(Xsing',Ysing',ax,ay,bx,by,cx,cy,dx,dy,PARAM.visc(i));
