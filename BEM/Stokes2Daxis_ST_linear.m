...
function [SXX,SYY,QXXX,QXXY,QXYY,QYYY,GXX_an,GYY_an] =...
                Stokes2Daxis_ST_linear(SXX,SYY,QXXX,QXXY,QXYY,QYYY,walls,globalX,globalY,X0,Y0,dL,el)
...
%        QYYY(1+6*(i-1):6*i,i+1) = zeros(6,1);
    ...
    %because first last element touches the axis first node SYMMETRY CONDITION
    dx1 = globalX(1+6*(el-1):6*el)-X0(1);
    dy1 = globalY(1+6*(el-1):6*el)-Y0(1);
    
    r2 = dxel.^2+dyel.^2;
    SXX(1+6*(el-1):6*el,el) = (dxel.*dxel)./r2;
    SYY(1+6*(el-1):6*el,el) = (dyel.*dyel)./r2;
    
    dxend = globalX(1+6*(el-1):6*el)-X0(walls+1);
    dyend = globalY(1+6*(el-1):6*el)-Y0(walls+1);
