...
function [GXX,GXY,GYX,GYY,A11,A12,A21,A22] = computeKernelStokesConst2d(x,y,x0,y0,indIn,indOut,ST)
    ...
    [SXX,SXY,SYY,QXXX,QXXY,QXYY,QYYY] = GT_2DStokes (globalXmatr,globalYmatr,X0matr,Y0matr);
    SYX = SXY;
      
    %singular treatment
    if ST==1
        range = indIn:indOut;
        [SXX(:,range),SYY(:,range),QXXX(:,range),QXXY(:,range),QXYY(:,range),QYYY(:,range),SXX_an,SYY_an] = stokesST2d(SXX(:,range),SYY(:,range),QXXX(:,range),QXXY(:,range),QXYY(:,range),QYYY(:,range),deltaL,globalXmatr(:,range),globalYmatr(:,range),x0(range),y0(range));
    end
    QXYX = QXXY;
    QYXX = QXXY;
    QYXY = QXYY;
