...
function [GXX,GXY,GYX,GYY,TXXX,TXXY,TXYX,TXYY,TYXX,TYXY,TYYX,TYYY] = computeGT_visu_2DStokes_interface(x,y,X0,Y0,fixed)
    ...
    XX = repmat([(x(2:fixed+1)+x(1:fixed))/2 (x(fixed+3:end)+x(fixed+2:end-1))/2],6,1);
    YY = repmat([(y(2:fixed+1)+y(1:fixed))/2 (y(fixed+3:end)+y(fixed+2:end-1))/2],6,1);
    globalX = XX(:)'+GPX;
    globalY = YY(:)'+GPY;
