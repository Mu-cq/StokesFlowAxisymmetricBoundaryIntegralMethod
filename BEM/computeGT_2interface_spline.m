...
function [GXX,GXY,GYX,GYY,A11,A12,A21,A22,T1,T2,D1,D2,Iaxis] = ...
    computeGT_2interface_spline(x,y,walls,drop1,r,ax,ay,bx,by,cx,cy,dx,dy)
...
%     hold off
    ...
    GXX(:,1:walls) = intSXX(6:6:end,:)'-subSXX' ...
        + [diag(2*(-2*deltaL(1:walls)/2.*log(deltaL(1:walls)/2)+3*deltaL(1:walls)/2)); zeros(el-walls+2,walls)];
