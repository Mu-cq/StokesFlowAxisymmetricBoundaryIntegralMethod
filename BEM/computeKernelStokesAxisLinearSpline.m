...
function [GXX,GXY,GYX,GYY,A11,A12,A21,A22] = computeKernelStokesAxisLinearSpline(x0,y0,indIn,indOut,ST,ax,ay,bx,by,cx,cy,dx,dy,kernelFreeSpace,wallPos)
...
%     plot(h,'o-')
    ...
    if ST==1
        ...
        GXX(range1,2:N-2) = GXX(range1,2:N-2) + anA1;
        GXX(range1,3:N-1) = GXX(range1,3:N-1) + anB1;
        GXX(range2,2:N-2) = GXX(range2,2:N-2) + anB2;
        GXX(range2,3:N-1) = GXX(range2,3:N-1) + anA2;
        
        GYY(range1,2:N-2) = GYY(range1,2:N-2) + anA1;
        GYY(range1,3:N-1) = GYY(range1,3:N-1) + anB1;
        GYY(range2,2:N-2) = GYY(range2,2:N-2) + anB2;
        GYY(range2,3:N-1) = GYY(range2,3:N-1) + anA2;
        
        %because singularity on the axis don't has to be treated
        GXX(range(2),1) = GXX(range(2),1) + 1/2*h1(1);
        GXX(range(2),2) = GXX(range(2),2) + 3/2*h1(1);
        GXX(range(end-1),end-1) = GXX(range(end-1),end-1) + 3/2*h0(elem);
        GXX(range(end-1),end) = GXX(range(end-1),end) + 1/2*h0(elem);
        
        GYY(range(2),1) = GYY(range(2),1) + 1/2*h1(1);
        GYY(range(2),2) = GYY(range(2),2) + 3/2*h1(1);
        GYY(range(end-1),end-1) = GYY(range(end-1),end-1) + 3/2*h0(elem);
        GYY(range(end-1),end) = GYY(range(end-1),end) + 1/2*h0(elem);
